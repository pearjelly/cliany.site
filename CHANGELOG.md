...
- Plain `browser extract --format text|markdown|json` now reads the current page with the supported page API. Markdown conversion uses a declared dependency; JSON parses the selected element's text, and missing selectors fail explicitly.
...
- `GET /doctor` now rejects repeated `require_capability` keys and `GET /verify` rejects repeated `domain` keys with `BAD_REQUEST` / HTTP `400`, instead of silently selecting one query value.
...
- The root CLI now recognizes a discovered current-schema adapter that cannot be registered and returns actionable `E_VERIFY_STATIC` details (`schema_error`, `commands_missing`, or `commands_unloadable`) instead of a generic unknown-command error. The response directs people and automation to `cliany-site verify <domain> --strict --json` before any browser or LLM work.
...
- Metadata schema v3 again accepts the framework's supported API-capability adapter metadata, including legacy string endpoint and command declarations, so `verify` can still enforce schema, security-scan, and manifest-hash checks for published API demos.
...
### Fixed
- `cliany_site.__version__` now reads installed distribution metadata instead of returning the stale `0.14.0` constant, so Python SDK consumers receive the same version as `cliany-site --version` and PyPI.
...
- Release readiness now requires the Candidate Promotion Runbook to keep the live LLM preflight, adapter package, metadata validation, online smoke, and `preflight_state.*` contract before a release can pass project metadata validation.
- Release readiness now requires `scripts/extract_doctor_preflight_evidence.py` to keep the doctor evidence hashes, `preflight_state`, Markdown blocker fields, and nonzero missing-evidence exit contract before release.
- Doctor preflight evidence now includes `summary.llm_live_preflight` and `checks[llm_live].details.status_code` across the cases CLI, planner artifacts, validator summaries, and standalone extractor, so candidate blockers include the compact readiness object and provider status code.
- Release readiness now blocks target release drafts that omit the `llm_live_preflight_not_ready` blocker reason code, keeping release notes aligned with the current doctor preflight state contract.
