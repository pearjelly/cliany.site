...
Use `cliany-site cases --json` to inspect active demos, candidate workflows, offline validation commands, and candidate promotion next actions from the CLI; `promotion_evidence_summary.primary_task_detail` points automation at the first candidate task to advance. Use `cliany-site cases --case-id pypi-project-search --json` to open one case with validation and promotion details; omit `--json` for a copy-friendly human handoff with Promotion Tasks. Add `--issue-template` to print a GitHub issue body for a candidate promotion task; combine it with `--json` to also read `issue_template_primary_task` without parsing Markdown. Add `--evidence-bundle` to print a structured local evidence checklist, and combine it with `--json` for a machine-readable evidence bundle.
