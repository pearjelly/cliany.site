...
For the doctor-guided active demo path, execute `data.summary.demo_adapter_quickstart.recommended_commands`, not the compatibility `commands` list: an occupied install target starts with `verify` and never triggers an automatic overwrite.
