...
- **Chrome 自动管理**：自动检测和启动 Chrome 调试实例，跨平台支持 macOS/Linux/Windows，无需手动配置
- **适配器增量合并**：重复 explore 同一网站时自动合并新旧 adapter，保留已有命令，智能处理冲突
- **原子命令系统**：从工作流中自动提取可复用的原子操作（登录、搜索等），跨 adapter 共享，参数化执行
- **录制流程验证**：多阶段验证器自动校验操作步骤，结构化错误报告，支持 --retry 重试
- **TUI 管理界面**：基于 Textual 的终端 UI，可视化管理适配器、查看操作日志、导入导出配置
- **活动日志追踪**：记录所有 explore/run 操作，便于审计和故障排查
...
| `tui` | | 启动 TUI 终端管理界面 |
| `atoms list` | `[--json]` | 列出所有已提取的原子命令 |
| `atoms run <name>` | `[--json] [args...]` | 执行指定原子命令 |
