...
可以先运行 `cliany-site doctor --llm-live --json > /tmp/cliany-doctor-preflight.json`，再运行 `cliany-site cases --case-id pypi-project-search --evidence-bundle --doctor-json /tmp/cliany-doctor-preflight.json --json` 或 `cliany-site cases --case-id pypi-project-search --issue-template --doctor-json /tmp/cliany-doctor-preflight.json`。这样 evidence bundle / issue body 会直接带上 `doctor_preflight_evidence_values`、`doctor_preflight_evidence_ok`、`doctor_preflight_evidence_missing_count`、`doctor_preflight_evidence_null_count`、`doctor_preflight_evidence_null_fields` 和 `doctor_preflight_state`，不用维护者手动运行 extractor 后再拼 blocker 评论。`null_fields` 表示 selector 存在但值为 JSON `null`，不会与真正缺失的 selector 混淆。
