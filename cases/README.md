...
已有的公开 candidate issue 也必须跟随当前 template。先运行 `python scripts/audit_candidate_issues.py --repo pearjelly/cliany.site --json` 只读审计 title、body 和 body SHA-256；它还会将不属于当前 candidate manifest 的开放 `case-proposal` issue 报告为 `unexpected`。不带 `--json` 时，unexpected 行会直接给出实际 title 和 URL，方便先定位阻断项。仅在 `stale` 项已经人工复核，且 `missing`、`duplicate`、`unexpected` 均已解决后，才运行 `python scripts/audit_candidate_issues.py --repo pearjelly/cliany.site --apply --confirm-rewrite --json`。该工具不会创建、关闭或提前晋级 issue。
