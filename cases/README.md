...
已有的公开 candidate issue 也必须跟随当前 template。先运行 `python scripts/audit_candidate_issues.py --repo pearjelly/cliany.site --json` 只读审计 title、body 和 body SHA-256；仅在 `stale` 项已经人工复核后，才运行 `python scripts/audit_candidate_issues.py --repo pearjelly/cliany.site --apply --confirm-rewrite --json`。该工具不会创建、关闭或提前晋级 issue；遇到 `missing` 或 `duplicate` 会保持失败，要求维护者先处理公开协作状态。
