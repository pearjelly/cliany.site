...
| `pypi-project-search` | PyPI 项目搜索 | candidate | Python 包注册表搜索候选；样例输出见 [pypi-project-search.json](examples/pypi-project-search.json)，[本地包与只读 smoke](../docs/user-evidence/2026-09-23-pypi-package-smoke.md) 已验证，待公开 release asset 后晋级 active |
...
如果 `cliany-site doctor --llm-live --require-capability generate_adapters --json` 显示 `generate_adapters.ready=false`，或 `llm_live` 返回 warning/error（例如 `E_LLM_UNAVAILABLE` 或 `E_UNKNOWN` connection error），维护者应停止本轮 `explore`，把 doctor JSON / 错误摘要贴回 issue，并让 `adapter_package` 保持 `pending` 或标记为 `blocked`；不要用失败的上游调用伪造成 adapter package 证据。
可以先运行 `cliany-site doctor --llm-live --require-capability generate_adapters --json > /tmp/cliany-doctor-preflight.json`，再运行 `cliany-site cases --case-id pypi-project-search --evidence-bundle --doctor-json /tmp/cliany-doctor-preflight.json --json` 或 `cliany-site cases --case-id pypi-project-search --issue-template --doctor-json /tmp/cliany-doctor-preflight.json`。这样 evidence bundle / issue body 会直接带上 `doctor_preflight_evidence_values`、`doctor_preflight_evidence_ok`、`doctor_preflight_evidence_missing_count`、`doctor_preflight_evidence_null_count`、`doctor_preflight_evidence_null_fields` 和 `doctor_preflight_state`，不用维护者手动运行 extractor 后再拼 blocker 评论。证据字段包含 `summary.capabilities.generate_adapters.local_ready` 和 `summary.capabilities.generate_adapters.local_blockers`，所以 live provider 故障时仍能单独证明本地前提是否健康；`null_fields` 表示 selector 存在但值为 JSON `null`，不会与真正缺失的 selector 混淆。
同一证据包也会输出 `doctor_preflight_state_fields` 和 `doctor_preflight_state_statuses`，让维护工具不用打开 extractor 源码也能解释 state。字段固定为 `preflight_state.status`、`preflight_state.ready_for_adapter_package`、`preflight_state.primary_reason`、`preflight_state.reason_codes` 和 `preflight_state.next_action`；状态只允许 `ready`、`blocked`、`missing_fields`。只有 `preflight_state.status=ready` 且 `preflight_state.ready_for_adapter_package=true` 时才继续真实 `explore`；否则先把 doctor blocker evidence 贴回 issue。
批量生成 candidate issue artifacts 时，也可以把同一份 doctor JSON 传给 `python scripts/plan_next_iteration.py --doctor-json /tmp/cliany-doctor-preflight.json --issues-dir /tmp/cliany-candidate-issues`；生成的 `issue-metadata.json` 和每个 candidate issue body 会保留当前 `doctor_preflight_state` 与带 `--doctor-json` 的 evidence bundle / issue template 命令。
...
创建 candidate promotion issue 时，正文可以复制 `--issue-template` 的输出；该模板会包含 `Primary Runbook`、LLM/doctor blocker comments、`Doctor Preflight Evidence Fields` 和带可粘贴占位符的 `Doctor Preflight Evidence Template`。同时加 `--json` 时，`issue_template_primary_task` 会带同一组 acceptance、runbook、preflight 字段、`doctor_preflight_evidence_template` 映射以及 `doctor_preflight_evidence_template_field_count` / `doctor_preflight_evidence_template_sha256`，方便 issue bot 不解析 Markdown 也能读取首要证据任务并比较模板是否漂移。附件或评论应附上 `--evidence-bundle --json` 的机器可读结果，例如 `cliany-site cases --case-id pypi-project-search --evidence-bundle --json`。当 `--doctor-json` 已附加预检时，issue 的 Primary Evidence Task 会把 package evidence 与 doctor preflight evidence 分开显示，并在 provider 暂不可用时明确要求恢复后重跑严格预检；它仍不会允许 candidate explore。普通 `cliany-site cases --status candidate` 输出也会在 `Candidate 下一步` 中显示 `preflight_required`、`preflight_blocker` 和 `runbook_first`，方便只读 human handoff 的维护者先确认 live LLM gate。它把 future adapter 命令明确标为当前不可运行，只有包已发布、安装且 `verify --strict` 通过后才可执行，不能当作 active demo 快速命令。这样后续维护者能直接对比 issue、case report 和 `promotion_evidence`，确认哪些晋级证据仍在 pending。

已有的公开 candidate issue 也必须跟随当前 template。先运行 `python scripts/audit_candidate_issues.py --repo pearjelly/cliany.site --json` 只读审计 title、body 和 body SHA-256；它还会将不属于当前 candidate manifest 的开放 `case-proposal` issue 报告为 `unexpected`。提供 `--doctor-json` 时，本地 JSON audit 仍记录 source path，但可公开的 issue body 只保留 evidence values 的 SHA-256、gate state 和抽取字段，不泄露维护机路径。不带 `--json` 时，unexpected 行会直接给出实际 title 和 URL，方便先定位阻断项。仅在 `stale` 项已经人工复核，且 `missing`、`duplicate`、`unexpected` 均已解决后，才运行 `python scripts/audit_candidate_issues.py --repo pearjelly/cliany.site --apply --confirm-rewrite --json`。该工具不会创建、关闭或提前晋级 issue。

Evidence bundle 的 JSON 会为每个任务输出 `acceptance_criteria`，并在顶层提供同名任务到验收条件的映射。`primary_next_task_acceptance_criteria` 会把当前首要任务需要贴的证据单独抬出来，避免自动化只知道要运行哪条命令，却不知道应该把什么结果作为完成证据。`primary_next_task_runbook` 会把首要任务拆成有序步骤；当首要任务是 `adapter_package` 时，第一步是 `cliany-site doctor --llm-live --require-capability generate_adapters --json`，之后才是 explore 命令和验收证据。JSON 顶层、`primary_next_task` 和 `adapter_package` 任务也会输出 `doctor_preflight_evidence_fields`、`doctor_preflight_state_fields` 和 `doctor_preflight_state_statuses`，方便维护者在 CDP 或 LLM gate 失败时直接贴回 `summary.capabilities.run_browser_workflows.ready`、`checks[cdp].status`、`checks[llm_live].details.error_code` 等 blocker 字段，并按 `ready` / `blocked` / `missing_fields` 判断是否继续真实 `explore`。人类 Markdown 输出也会展示 `Acceptance criteria` 和 `Primary next runbook` 小节，方便直接复制到 GitHub issue 评论。
