...
- 将 `search-extraction-gap` 拆成抽取能力设计任务：列表检测、字段映射、任务级验收。当前已先在保存的抽取 Markdown 报告和生成 adapter 的 JSON 输出中标记空结果、全空字段和部分缺字段，并让 `list-` / `search-` 数据命令在质量为空时返回 `E_EMPTY_RESULT`；后续继续收紧部分缺字段的验收语义。
