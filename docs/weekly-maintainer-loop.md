...
`artifact_bundle_summary` 也会带上 `case_promotion_evidence_candidate_count`、`case_promotion_evidence_task_count`、`case_promotion_evidence_pending_count`、`case_promotion_evidence_blocked_count`、`case_promotion_evidence_complete_count` 和 `case_promotion_evidence_primary_next_action`，让只读整包摘要的维护工具能展示 candidate 晋级证据规模和首要动作，而不必展开完整 task 明细。
