...
`artifact_bundle_summary` 也会带上 `case_promotion_evidence_candidate_count`、`case_promotion_evidence_task_count`、`case_promotion_evidence_pending_count`、`case_promotion_evidence_blocked_count`、`case_promotion_evidence_complete_count`、`case_promotion_evidence_primary_next_action`、`case_promotion_evidence_primary_case_id`、`case_promotion_evidence_primary_task`、`case_promotion_evidence_primary_status` 和 `case_promotion_evidence_primary_evidence_sha256`，让只读整包摘要的维护工具能展示 candidate 晋级证据规模、首要动作、对应 case/task/status 和当前证据 hash，而不必展开完整 task 明细。
