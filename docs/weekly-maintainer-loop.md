...
Candidate issue artifacts `README.md` 会在 `Artifact Bundle Summary` 前展示 `Candidate Issue Gate Quick Summary`，把 gate status、`can_create_issues`、`requires_maintainer_review`、reason/action 数量、primary reason/action、latest tag 和 visibility 放到维护者最容易先读到的位置。
