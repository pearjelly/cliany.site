...
python scripts/plan_next_iteration.py --handoff-json
...
python scripts/plan_next_iteration.py --doctor-json /tmp/cliany-doctor-preflight.json --issues-dir /tmp/cliany-candidate-issues
...
python scripts/audit_candidate_issues.py --repo pearjelly/cliany.site --json
...
GitHub GraphQL 返回 `EOF` 时同样按可重试的 `E_GITHUB_UNAVAILABLE` 处理；先等待 API 恢复并重新读取 issue 状态，不要把连接失败判为 `missing` 或执行 `--apply`。
...
Candidate promotion 的 issue template、evidence bundle、promotion plan、case validation summary 和 planner artifacts 也会输出 `expected_adapter_package`，例如 `pypi.org-<version>.cliany-adapter.tar.gz`。维护者可以把这个字段当作 release asset / packages-dir 校验的稳定包名模板，而不必从 `adapter_package` 的自然语言描述里解析。

推进首要 candidate 时，先按 [Candidate Promotion Runbook](candidate-promotion-runbook.md) 执行 live LLM preflight、adapter package、metadata validation 和 online smoke 四步；如果 daily release cap 已满，只准备 issue/evidence，不创建新 tag。PyPI 当前首要案例的包名模板应保持 `pypi.org-<version>.cliany-adapter.tar.gz`，并在 `promotion_evidence` 更新前通过 `python scripts/validate_cases.py --packages-dir ~/.cliany-site/packages --include-candidate-packages --strict` 复核。

`cliany-site cases --case-id <id> --evidence-bundle --json`、candidate issue body 和 `issue-metadata.json` 也会输出 `promotion_command_plan`，把 candidate 晋级执行顺序拆成 `llm_live_preflight`、`adapter_package`、`metadata_validation`、`online_smoke` 四条命令；其中后三条仍是决定能否晋级 active 的真实证据。Evidence bundle 还会输出 `primary_next_task_runbook`、`primary_next_task_runbook_first_step`、`primary_next_task_runbook_first_command` 和 `primary_next_task_runbook_first_command_sha256`，promotion plan 会输出 `primary_runbook`，把首要任务的 preflight、执行命令和验收证据排成可执行步骤；`plan_next_iteration.py --issues-dir` 生成的 `candidate_promotions`、`issue-metadata.json`、issue body 和 artifacts `README.md` 也会保留 `evidence_bundle_primary_next_task_runbook`；`scripts/validate_cases.py --json` 会在 `promotion_evidence_summary.primary_next_task_runbook` 暴露同一结构，并在 summary 层暴露首个 step/command/hash，`scripts/validate_cases.py --report ...` 和 `scripts/release_readiness.py --report ...` 也会渲染 `Candidate Primary Runbook` 小节。只读 evidence bundle、planner JSON、case validation artifact、release readiness report 或 issue artifacts 的维护工具不必从自然语言 next action 推断该运行哪条命令，也不必猜测是否要先跑 live LLM preflight。

同一 `promotion_command_plan` 的每个条目也包含 `promotion_command_plan[*].command_sha256`，让只读 evidence bundle、planner artifacts 或 issue metadata 的工具可以检查四条晋级命令是否漂移，而不用解析 runbook 文本。

如果维护工具只展开 primary task，而不展开完整 runbook，可以直接读取 `llm_live_preflight_required`、`llm_live_preflight_command`、`llm_live_preflight_command_sha256`、`llm_live_preflight_blocker_note`、`llm_live_preflight_evidence_fields`、`doctor_preflight_evidence_fields`、`doctor_preflight_evidence_template_field_count`、`doctor_preflight_evidence_template_sha256`、`doctor_preflight_state_fields` 和 `doctor_preflight_state_statuses`。`adapter_package` task 会把 `llm_live_preflight_required` 设为 `true`，并给出 extractor `preflight_state.status`、`preflight_state.ready_for_adapter_package`、`preflight_state.primary_reason`、`preflight_state.reason_codes`、`preflight_state.next_action` 的稳定读取契约；`metadata_validation` 和 `online_smoke` 会设为 `false`，避免把“不需要 preflight”和“字段缺失”混在一起。面向人工交接的 `cliany-site cases --status candidate` 也会在 `Candidate 下一步` 中显示 `preflight_required`、`preflight_blocker` 和 `runbook_first`，让维护者不用展开 JSON 也能先处理 live LLM gate。`scripts/plan_next_iteration.py --json`、`--report` 和 issue artifacts README 还会把首要任务的 preflight gate 提升为 `case_promotion_evidence_primary_llm_live_preflight_required`、`case_promotion_evidence_primary_llm_live_preflight_command`、`case_promotion_evidence_primary_llm_live_preflight_command_sha256`、`case_promotion_evidence_primary_llm_live_preflight_blocker_note`、`case_promotion_evidence_primary_llm_live_preflight_blocker_comment`、`case_promotion_evidence_primary_doctor_preflight_blocker_comment` 和 `case_promotion_evidence_primary_doctor_preflight_evidence_template_sha256`，方便只读 summary 的工具不展开 primary task 也能判断是否应先跑 doctor，并在 CDP 或 LLM gate 失败时复制标准 blocker comment。

如果维护工具只读取 `cliany-site cases --status candidate --promotion-plan --json` 的候选队列，也可以直接读取 `promotion_plan.primary_doctor_preflight_evidence_template_field_count`、`promotion_plan.primary_doctor_preflight_evidence_template_sha256`、candidate `primary_doctor_preflight_evidence_template_sha256` 和 `task_queue[*].doctor_preflight_evidence_template_sha256`，无需展开完整 evidence bundle 即可比对 doctor evidence template 是否漂移。

当 live LLM preflight 未通过时，`cliany-site cases --case-id <id> --evidence-bundle --json` 会在 `llm_live_preflight_evidence_fields` 中列出应从 doctor JSON 贴回的字段：`summary.ready_for_explore`、`summary.llm_live_preflight`、`summary.capabilities.generate_adapters.ready`、`checks[llm_live].status`、`checks[llm_live].details.error_code`、`checks[llm_live].details.retryable`、`checks[llm_live].details.status_code`、`checks[llm_live].details.phase` 和 `checks[llm_live].details.message`。同一 evidence bundle 还会输出 `doctor_preflight_evidence_selectors`，把 `checks[llm_live].details.error_code` 映射为实际 JSON selector `data.checks[name="llm_live"].details.error_code`，明确 `checks` 是按 `name` 选择的列表。Candidate issue body、`issue-metadata.json` 和 artifacts `README.md` 的 Candidate Summary 也会渲染同一组字段；`scripts/validate_cases.py --json` / `--report` 会在 `promotion_evidence_summary` 和 `LLM Live Preflight Evidence Fields` 小节暴露同一字段清单、数量与 hash，并在 `promotion_evidence_summary.primary_next_task.doctor_preflight_evidence_template_sha256` 以及 report 表格行 `primary_doctor_preflight_evidence_template_sha256` 中输出 doctor template 漂移信号；纯文本 `scripts/validate_cases.py --strict` stdout 也会打印 `promotion_evidence_primary_doctor_preflight_evidence_template_field_count`、`promotion_evidence_primary_doctor_preflight_evidence_template_sha256` 和 `promotion_evidence_primary_llm_live_preflight_command_sha256`，让 release/CI 日志不用 JSON artifact 也能复核同一模板和 preflight 命令；`scripts/release_readiness.py --json` / `--report` 和 `scripts/plan_next_iteration.py --json` / `--report` 会把它提升为 `case_promotion_llm_live_preflight_evidence_fields`、`case_promotion_llm_live_preflight_evidence_field_count` 和 `case_promotion_llm_live_preflight_evidence_fields_sha256`。`plan_next_iteration.py --issues-dir` 也会把这组别名写入 `artifact-manifest.json`、`artifact_bundle_summary` 和 artifacts `README.md`，并额外写入 `case_promotion_doctor_preflight_evidence_template_sha256` 与 `case_promotion_evidence_primary_llm_live_preflight_command_sha256` 来比对 doctor template 和 preflight 命令是否漂移。这样维护者可以把 `summary.llm_live_preflight.ready=false`、`E_LLM_UNAVAILABLE` provider connection failure 或其他 warning/error 记录为 blocker 证据，而不是继续执行真实 `explore`。

如果工具只读取摘要层，优先读取 `case_promotion_evidence_primary_runbook_step_count`、`case_promotion_evidence_primary_runbook_steps`、`case_promotion_evidence_primary_runbook_steps_sha256`、`case_promotion_evidence_primary_runbook_first_step`、`case_promotion_evidence_primary_runbook_first_command` 和 `case_promotion_evidence_primary_runbook_first_command_sha256`。这些字段会出现在 release readiness JSON、planner JSON、planner 文本/Markdown report、candidate issue artifacts 的 `artifact_bundle_summary` 和 artifacts `README.md` 中；需要完整 handoff 时再展开 `case_promotion_evidence_primary_runbook` 或 `evidence_bundle_primary_next_task_runbook`。
...
当目标 tag 因 daily release cap 暂停时，candidate issue artifacts 也会在 `artifact-manifest.json`、`artifact_bundle_summary`、`publication-handoff.json`、`Candidate Issue Gate Quick Summary` 和 `Publication Handoff` 中同步展示 `daily_release_cap_blocked`、`daily_release_resume_date`、`daily_release_resume_date_sha256`、`daily_release_resume_command` 和 `daily_release_resume_command_sha256`，让维护者从候选任务交接包就能看到下一次允许尝试 tag 的日期、恢复当天应先执行的 strict readiness 命令，以及命令漂移 hash。

`plan_next_iteration.py --handoff-json` 的 `primary_candidate` 也会携带完整 `promotion_command_plan`、`promotion_command_plan_count`、`promotion_command_plan_sha256`、`promotion_command_plan_task_names`、`promotion_command_plan_task_names_sha256`、`promotion_command_plan_first_command`、`promotion_command_plan_first_command_sha256`、`promotion_command_plan_summary`、`promotion_command_plan_summary_sha256`、`issue_template_command_sha256` 和 `evidence_bundle_json_command_sha256`。只读 compact handoff 的 issue bot 可以直接按 `llm_live_preflight`、`adapter_package`、`metadata_validation`、`online_smoke` 四步命令展示候选案例晋级路径，显示第一条应执行命令，并用命令计划、issue 模板命令和 evidence bundle JSON 命令 hash 判断 issue 模板、evidence bundle 或 candidate manifest 是否漂移。
...
`release_readiness.py --json` 和 `--report` 也会输出 `publication_summary` 与 `publication_summary_sha256`。维护者优先看 `publication_summary.status`、`publication_summary.worktree_clean`、`publication_summary.ahead_count`、`publication_summary.latest_tag`、`publication_summary.target_tag`、`publication_summary.target_tag_status`、`publication_summary.target_tag_release_gate_status`、`publication_summary.target_tag_release_gate_blocker_count`、`publication_summary.target_tag_primary_command`、`publication_summary.primary_next_action` 和 `publication_summary.primary_publish_command`，就能判断当前是 `published` 还是 `blocked`，本地 worktree 是否挡住发布，目标 release tag 是什么，目标 tag 是否仍 `blocked_by_readiness`，以及第一条该执行的发布动作和发布命令；当 daily cap 暂停目标 tag 时，同一个 readiness JSON / Markdown report 也会在顶层输出 `daily_release_resume_command` 和 `daily_release_resume_command_sha256`，与 `daily_release_resume_date` 一起给出恢复当天应运行的 strict readiness 命令和命令漂移 hash；只读发版预检 artifact 的工具也可以直接读取顶层别名 `publication_summary_primary_next_action` 与 `publication_summary_primary_publish_command`，再用 `publication_summary_sha256`、`publication_next_actions_sha256`、`publication_publish_commands_sha256`、`target_tag_commands_sha256`、`daily_release_resume_command_sha256` 和 `target_tag_release_gate_blockers_sha256` 检测这组摘要、发布待办列表、发布命令列表、目标 tag 命令、daily cap 恢复命令和 readiness blocker 列表是否漂移，按需展开 `publication_ref_context`、`publication_next_actions`、`publication_publish_commands`、`publication_tag_publish_decision.target_tag_commands` 或 `publication_tag_publish_decision.target_tag_release_gate_blockers`。

提交后进入发布流程时，维护者先读取 `release_readiness.py --json` 或 Markdown report 中的 `standard_release_flow`、`standard_release_flow_status`、`standard_release_flow_primary_next_action`、`standard_release_flow_step_count`、`standard_release_flow_step_names`、`standard_release_flow_step_names_sha256`、`standard_release_flow_steps_sha256`、`standard_release_flow_first_step_name`、`standard_release_flow_last_step_name`、`standard_release_flow_step_boundary_sha256`、`standard_release_flow_step_status_counts`、`standard_release_flow_step_status_counts_sha256`、`standard_release_flow_primary_blocked_step_name`、`standard_release_flow_primary_pending_step_name`、`standard_release_flow_has_website_deploy`、`standard_release_flow_website_deploy_command`、`standard_release_flow_website_deploy_command_sha256`、`standard_release_flow_has_website_inspect`、`standard_release_flow_website_inspect_command`、`standard_release_flow_website_inspect_command_sha256`、`standard_release_flow_has_distribution_audit`、`standard_release_flow_distribution_audit_command`、`standard_release_flow_distribution_audit_command_sha256`、`standard_release_flow_commands_sha256` 和 `standard_release_flow_sha256`。这组字段把标准发版动作压缩成可审阅步骤：严格 release readiness、整理 `CHANGELOG.md`、更新 `pyproject.toml`、离线验证、推送分支、确认 GitHub CI 与 Embodied CI、创建目标 tag、运行 tagged readiness 后推送目标 tag、用 `cd site && vercel link --yes --project cliany.site && vercel --prod --yes` 部署官网生产环境、用 `cd site && vercel inspect www.cliany.site --wait --timeout 90s` 复核官网 alias，以及 `python scripts/check_release_publication.py --remote --distribution --json` 公开渠道复核；如果使用自定义 `--remote-name`，strict readiness 和远端复核命令都会保留它。只读摘要字段的工具先检查 `standard_release_flow_first_step_name`、`standard_release_flow_last_step_name`、`standard_release_flow_step_names`、`standard_release_flow_step_status_counts`、`standard_release_flow_steps_sha256`、`standard_release_flow_has_website_deploy=true`、官网部署命令 hash、`standard_release_flow_has_website_inspect=true`、官网 inspect 命令 hash、`standard_release_flow_has_distribution_audit=true` 和 distribution audit 命令 hash；如果 `standard_release_flow_primary_blocked_step_name` 有值，先定位该步骤，再展开完整步骤。如果 status 仍是 `blocked`，先执行 primary next action，不要跳过 gate 直接 tag。
...
使用 `--issues-dir` 生成候选任务交接包时，`artifact-manifest.json`、`publication-handoff.json`、artifacts `README.md` 的 `Publication Handoff` 和 `artifact_bundle_summary` 也会展示 standard release flow 摘要，包括 `standard_release_flow_status`、`standard_release_flow_primary_next_action`、`standard_release_flow_command_count`、`standard_release_flow_step_count`、`standard_release_flow_step_names`、`standard_release_flow_step_names_sha256`、`standard_release_flow_steps_sha256`、`standard_release_flow_first_step_name`、`standard_release_flow_last_step_name`、`standard_release_flow_step_boundary_sha256`、`standard_release_flow_step_status_counts`、`standard_release_flow_step_status_counts_sha256`、`standard_release_flow_primary_blocked_step_name`、`standard_release_flow_primary_pending_step_name`、`standard_release_flow_has_website_deploy`、`standard_release_flow_website_deploy_command`、`standard_release_flow_website_deploy_command_sha256`、`standard_release_flow_has_website_inspect`、`standard_release_flow_website_inspect_command`、`standard_release_flow_website_inspect_command_sha256`、`standard_release_flow_has_distribution_audit`、`standard_release_flow_distribution_audit_command`、`standard_release_flow_distribution_audit_command_sha256`、`standard_release_flow_commands_sha256` 和 `standard_release_flow_sha256`。只读 artifacts 的维护工具可以不展开 release readiness report，也能先判断标准发版 gate 是否仍是 `blocked`，确认步骤首尾、顺序和状态分布没有漂移，直接看到首个阻塞/待办步骤，并确认官网部署命令、官网 inspect 命令与最终 distribution audit 命令都已纳入候选任务交接包。

如果摘要 hash 有变化，再展开 `standard_release_flow.steps` 审计执行顺序。Planner 在无法复用 release readiness 完整 flow 时也会自行生成步骤列表，保持 `publish_branch` → `ci_verification` → `target_tag` → `website_deploy` → `website_inspect` → `remote_publication_audit` 的顺序可见。
...
发布 workflow 完成后，用 `python scripts/check_release_publication.py --remote --distribution --json` 做最终公开渠道审计。`--remote` 证明 branch/tag refs 已经公开，`--distribution` 额外读取 GitHub Release latest tag、PyPI project latest 和 PyPI version-specific JSON；自动化优先看 `distribution_ok`、`distribution.github_release_tag`、`distribution.pypi_version`、`distribution.pypi_latest_version`、`distribution.pypi_release_version` 和 `distribution.next_actions_sha256`，确保标准发版流程不只停在 git tag 可见，而是真的完成 GitHub Release 与 PyPI 发布。若 project latest 仍是旧版本但 version-specific endpoint 已有目标版本和文件，`pypi_version` 会使用目标版本，`pypi_latest_version` 会保留旧值以说明这是缓存滞后。
