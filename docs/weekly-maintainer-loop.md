...
当目标 tag 因 daily release cap 暂停时，candidate issue artifacts 也会在 `artifact-manifest.json`、`artifact_bundle_summary`、`publication-handoff.json`、`Candidate Issue Gate Quick Summary` 和 `Publication Handoff` 中同步展示 `daily_release_cap_blocked`、`daily_release_resume_date` 和 `daily_release_resume_date_sha256`，让维护者从候选任务交接包就能看到下一次允许尝试 tag 的日期。
