...
`artifact-manifest.json` 的 `candidate_issue_gate` 会和 `publication-handoff.json` 同源，用 `can_create_issues` 把硬性 publication preflight 和 release draft 人工审阅区分开。
