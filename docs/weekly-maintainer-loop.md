...
如果已经保存 `cliany-site doctor --llm-live --json > /tmp/cliany-doctor-preflight.json`，把 `--doctor-json /tmp/cliany-doctor-preflight.json` 一起传给 `plan_next_iteration.py --issues-dir`。生成的 `issue-metadata.json` 和 candidate issue body 会直接包含 `doctor_preflight_state`、extracted values、source path 和带 `--doctor-json` 的 issue/evidence bundle commands；当 live LLM 仍 blocked 时，维护者可以先创建 blocker-ready issue 草稿，不需要手工拼接 single-case evidence bundle。
