...
如果已经保存 `cliany-site doctor --llm-live --require-capability generate_adapters --json > /tmp/cliany-doctor-preflight.json`，把 `--doctor-json /tmp/cliany-doctor-preflight.json` 一起传给 `plan_next_iteration.py --issues-dir`。生成的 `issue-metadata.json` 和 candidate issue body 会直接包含 `doctor_preflight_state`、extracted values、source path 和带 `--doctor-json` 的 issue/evidence bundle commands；当 live LLM 仍 blocked 时，维护者可以先创建 blocker-ready issue 草稿，不需要手工拼接 single-case evidence bundle。

公开 candidate issue 创建后也会漂移。`python scripts/audit_candidate_issues.py --repo pearjelly/cliany.site --json` 使用当前 `cases --issue-template` body 审计每个开放 `case-proposal` issue；报告 `current`、`stale`、`missing`、`duplicate` 或 `unexpected`，并给出 body SHA-256。保存过 doctor JSON 时，追加 `--doctor-json /tmp/cliany-doctor-preflight.json`，使期望正文复用同一份 extracted gate evidence；JSON 报告会给出 source path、values SHA-256、state status 和 package runnable 状态，不能把旧的占位模板误当成当前 blocker handoff。`unexpected` 表示公开 label 存在、但 title 不再属于当前 candidate manifest。只有逐项审阅 `stale`，并先解决 `missing`、`duplicate`、`unexpected` 后才允许追加 `--apply --confirm-rewrite`；该显式操作只重写已匹配的 stale body，不会自动创建、关闭或把案例改为 `active`。
