...
`candidate_issue_gate` 本体也会带上 `primary_reason_code`、`primary_reason_description` 和 `primary_required_action`，artifacts `README.md` 的 `Publication Handoff` 会同步展示 `gate_primary_reason_code`、`gate_primary_reason_description` 和 `gate_primary_required_action`。只读取 gate 或 handoff 的工具可以直接渲染首要原因与动作，不必解析 `reason_codes[0]` 或 `required_actions[0]`。
