...
`plan_next_iteration.py --handoff-json` 的 `primary_candidate` 也会携带完整 `promotion_command_plan`、`promotion_command_plan_count`、`promotion_command_plan_sha256`、`promotion_command_plan_summary` 和 `promotion_command_plan_summary_sha256`。只读 compact handoff 的 issue bot 可以直接按 `llm_live_preflight`、`adapter_package`、`metadata_validation`、`online_smoke` 四步命令展示候选案例晋级路径，并用命令计划 hash 判断 issue 模板、evidence bundle 或 candidate manifest 是否漂移。
