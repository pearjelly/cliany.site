...
| 发布节奏 | 已发布 v0.14.2 | 每天至少 1 个可验证版本；每周至少 3 天有提交 |
...
> 版本命名遵循 semver。用户可见能力或行为变化用 minor，纯文档/修复用 patch。默认节奏是每天至少发布一个小版本；如果当天只有文档、门禁或证据交接改进，也发布 patch。
...
每日发布以“小而可验证”为原则：先读 readiness/plan 输出，再选择一条能在当天闭环的切片，最后按标准发版流程公开到 GitHub Release、PyPI 和官网。每周三类提交日仍保留，用来保证更长周期的健康度：
