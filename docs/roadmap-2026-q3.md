...
- Adapter 文件边界：严格 verify、根 CLI 和 SDK/API 的运行前预检会在读取或导入前拒绝符号链接 adapter 目录、核心文件和 manifest 声明文件，以 `security_issue` / `E_VERIFY_STATIC` 明确本地安装需重装，不把目录外内容误作已验证 adapter。存在 manifest 时，根 CLI、`execute` 与 `POST /execute` 都会在导入命令或启动浏览器前复用 manifest 完整性检查，拒绝声明文件符号链接和哈希不匹配。
