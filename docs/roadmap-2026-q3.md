...
2026-06-10 的原始路线图以 v0.14.2 为基线；当前公开基线为 v0.16.358。v0.16.357 加入真实浏览器端到端回归覆盖的执行修复、Cookie 主机范围保存，以及显式授权的 Jev 只读意图定位；v0.16.358 使实时模型连接故障的 JSON 建议更具体，并在官网公开 Jev 的授权边界。真实模型质量仍待验证。过去几周的实际进展已经提前完成了原计划中的多项基础建设：
