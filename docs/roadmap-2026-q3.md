...
2026-06-10 的原始路线图以 v0.14.2 为基线；到 2026-07-01 已发布到 v0.16.250。过去几周的实际进展已经提前完成了原计划中的多项基础建设：
...
| 发布节奏 | v0.16.250，GitHub Release / PyPI / 官网发布流程已跑通 | 每天 1~3 个可验证版本；每周至少 3 天有提交；发布后 publication audit 为绿 |
...
| 周期 | 主题 | 主要交付 | 验收标准 |
|------|------|----------|----------|
| 2026-06-10 ~ 2026-06-21 | 基础设施成型 | 首次成功路径、cases 命令、发布门禁、LLM preflight、CI 稳定化 | 已发布到 v0.16.250；Release/CI/Embodied CI 与 publication audit 全绿 |
...
2. 运行 `cliany-site doctor --llm-live --json`，如果返回 `E_LLM_UNAVAILABLE` provider connection failure 或 `generate_adapters.ready=false`，记录 blocker，不伪造 adapter package evidence。
