...
2026-06-10 的原始路线图以 v0.14.2 为基线；当前公开基线为 v0.16.354。过去几周的实际进展已经提前完成了原计划中的多项基础建设：
...
- API 查询参数契约：`GET /adapters?detail=` 与 `GET /doctor?llm_live=` 共用严格布尔解析；无效值稳定返回 `BAD_REQUEST` / HTTP `400`，不再静默退回简略 adapter 列表。
- API 查询歧义边界：上述布尔键各只能出现一次；重复或冲突值稳定返回 `BAD_REQUEST` / HTTP `400`，不依赖 query-string 的参数顺序。
...
- SDK session-host 边界：`save_session(domain)` 只接受带可选合法端口的主机、host:port 或 bracketed IPv6 host；空值、路径、URL 凭据、空白和无效端口会在连接 Chrome 或写 session 前返回 `E_INVALID_PARAM`，不把不受约束的标识带入浏览器和持久化路径。
- SDK 同步 session 入口：`save_session(domain, cdp_url=..., headless=...)` 为非 async 调用方复用同一条已校验的 session-host 与浏览器配置路径，不需要自行管理事件循环，也不会绕过 Chrome 或持久化安全边界。
...
2026-09-22：先修复 live preflight 的 JSON 模式兼容性；配置中的 provider 仍有 TLS 连接故障。下一次优先复核连通性并推进 PyPI 案例，若仍失败则复测已发布 active demo 的真实只读回放。9 月 9 日至 21 日没有新版本，日更目标未达成，需要恢复每日有实质改进且通过验证的发布节奏。
...
| 发布节奏 | v0.16.354，GitHub Release / PyPI / 官网发布流程已跑通，PyPI latest 缓存滞后可由版本专属 endpoint 复核，官网 alias 由 `vercel inspect www.cliany.site` 复核，短暂远端传输失败有有限重试而持续失败仍阻断，首页与文档无 Google Fonts 运行时依赖，API 查询与写请求的非法、重复或未知字段都在 tag 前由回归保护，直接 SDK 的 browser-facing URL、workflow 文本、doctor live-preflight 参数与 session host 也会在资源创建前由回归保护，primary adapter handoff command aliases 与 doctor preflight evidence state 已进入维护者路径 | 每天 1~3 个可验证版本；每周至少 3 天有提交；发布后 publication audit 为绿 |
