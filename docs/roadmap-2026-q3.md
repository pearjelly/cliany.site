...
| 发布节奏 | 已发布 v0.14.2 | 每天 1~3 个可验证版本；每周至少 3 天有提交 |
...
> 版本命名遵循 semver。用户可见能力或行为变化用 minor，纯文档/修复用 patch。默认节奏是每天至少发布一个小版本、最多发布三个版本；如果当天只有文档、门禁或证据交接改进，也发布 patch。达到每日上限后，只继续做下一版计划、验证和未发布准备。
...
每日发布以“小而可验证”为原则：先读 readiness/plan 输出，再选择一条能在当天闭环的切片，最后按标准发版流程公开到 GitHub Release、PyPI 和官网。每天发布 1~3 个版本；当 `daily_release_limit_ok=false` 或 `release_count_today` 已达到 `max_daily_releases`，当天不再创建 tag。每周三类提交日仍保留，用来保证更长周期的健康度：
...
如果遇到紧急 bug，可在任意日期追加 patch；如果当天没有用户可见代码改动，也可以发布文档、案例证据、发版门禁或官网同步 patch，但必须说明用户价值、验证方式和风险。
