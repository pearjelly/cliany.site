...
Keep `pypi-project-search` as `candidate` until the exact archive is attached to GitHub Release `v0.16.360`, its public SHA-256 is verified, and an independent HOME can install the asset by HTTPS URL and repeat strict verification plus the read-only smoke.
