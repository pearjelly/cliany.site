...
- `demo_adapter_quickstart`：当 `available=true`、`deprecated=false` 时，兼容字段 `commands` 保留固定 HTTPS + SHA-256 安装、诊断性 `verify` 和只读案例命令；`verify_command` 也继续保留该退出码为零的报告契约。实际执行 `recommended_commands`：`adapter_present=false` 时它包含安装、`verify --strict` 和只读命令；同名安装目标已占用时 `adapter_present=true`，它从 `verify --strict` 开始，不会重复安装或覆盖已有 adapter。严格验证会在 schema、安全扫描、manifest、legacy adapter 或显式 smoke 验证失败时以非零退出并在 error.details 保留诊断结果，顺序脚本不得继续执行只读命令。占用目标不代表 adapter 健康；即使目标是损坏目录或普通文件，也先避免重复安装并让严格验证给出实际诊断。它只会选择 `active`、无需登录的案例，不会把 candidate 当成可安装 demo。若 `deprecated=true` 或 `available=false`，不要执行其中的命令；读取 `replacement` 并改用 `case_catalog_quickstart`。
