...
`validate_cases.py` 会检查 `cases/manifest.json` 的结构、文档链接、active 案例命令和验证说明；传入 `--packages-dir ~/.cliany-site/packages` 时，还会离线检查 demo adapter 包中的 `manifest.json`、tarball 安全路径、声明文件哈希和 metadata schema v3。
