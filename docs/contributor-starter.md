...
维护者创建 GitHub issue 时，可以直接从 [Good First Issues](good-first-issues.md) 候选池选择任务；每个任务都带推荐验证命令，candidate promotion 的 Issue Body Template 会保留 `Reproduction Context` 和 `Evidence Bundle`，便于首次贡献者按 PR 模板提交证据。
