...
若由 doctor 引导 active demo，执行 `data.summary.demo_adapter_quickstart.recommended_commands`，不要直接执行兼容字段 `commands`：它使用 `verify --strict`，静态校验失败会以非零退出，顺序脚本不会继续执行后面的只读命令。安装目标已占用时它从这个门禁开始，绝不会自动覆盖；兼容字段 `verify_command` 仍保留诊断性、退出码为零的报告路径。
