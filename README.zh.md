...
Candidate 晋级时，如果 `generate_adapters.ready=false`，或 `llm_live` 返回 warning/error（例如 `E_LLM_UNAVAILABLE` provider connection failure），这只能作为 blocker 证据；`adapter_package` 应保持 `pending` 或标记 `blocked`，不要伪造 adapter package 证明。
