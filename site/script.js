const I18N = {
  ...
  'qs.contribute.validate': {
    zh: '候选案例应关联 <code>cases/manifest.json</code>、<code>cases/examples/</code>、<code>python scripts/validate_cases.py --strict</code>、带 <code>llm_live_preflight</code> 的 <code>cliany-site cases --status candidate --promotion-plan</code>、带 <code>primary_next_task_acceptance_criteria</code> 的 <code>cliany-site cases --json</code>，以及带 <code>Primary Acceptance Criteria</code> 的 <code>python scripts/plan_next_iteration.py --issues-dir /tmp/cliany-candidate-issues</code>。',
    en: 'Candidate cases should point to <code>cases/manifest.json</code>, <code>cases/examples/</code>, <code>python scripts/validate_cases.py --strict</code>, <code>cliany-site cases --status candidate --promotion-plan</code> with <code>llm_live_preflight</code>, <code>cliany-site cases --json</code> with <code>primary_next_task_acceptance_criteria</code>, and <code>python scripts/plan_next_iteration.py --issues-dir /tmp/cliany-candidate-issues</code> with <code>Primary Acceptance Criteria</code>.'
  },
  ...
  'qs.maintainer.desc': {
    zh: '当前基线：v0.16.248。使用每天 1~3 个版本的发布循环、每周维护者循环和 release readiness 的 next_actions，把路线图拆成小而可验证的发布切片。',
    en: 'Current baseline: v0.16.248. Use the 1-3 releases/day loop, weekly maintainer loop, and release readiness next_actions to turn the roadmap into small verified releases.'
  },
  'qs.maintainer.loop': {
