const I18N = {
  ...
  'try.subtitle': { zh: '安装、检查就绪状态，然后从公开只读案例拿到真实结果。', en: 'Install, check readiness, then get a real result from a public read-only case.' },
  ...
  'try.cases.title': { zh: '拿到真实结果', en: 'Get a real result' },
  'try.cases.body': { zh: '运行固定哈希的只读 Jira 案例。安装与严格校验通过后才查询，并要求至少一条 issue。', en: 'Run a pinned, read-only Jira case. Installation and strict verification happen before the query; at least one issue is required.' },
  ...
  'demo.quality.contract': {
    zh: '生成的 list/search/read/extract 命令及包含 extract action 的命令默认 expects_nonempty=true；零条数据、缺失字段或 partial 结果都会失败。失败的结构化对象行会在 data.quality.field_blank_rows 中按字段列出 1 起始结果行号。只有零匹配本来合法时才声明 expects_nonempty=false；已安装的旧 adapter 不会被静默改写。',
    en: 'Generated list/search/read/extract commands, and any command with an extract action, default to expects_nonempty=true; empty, missing, or partial data fails. Failed structured object rows list 1-based result numbers by field in data.quality.field_blank_rows. Declare expects_nonempty=false only when zero matches are valid; installed adapters are not silently rewritten.'
  },
  ...
  'qs.subtitle': { zh: '先从公开只读案例拿到结果，再为自己的命令配置 LLM', en: 'Run a public read-only result before configuring an LLM for your own commands' },
  'qs.step1': { zh: 'Step 1: 安装', en: 'Step 1: Install' },
  'qs.step2': { zh: 'Step 2: 检查就绪状态', en: 'Step 2: Check Readiness' },
  'qs.step3': { zh: 'Step 3: 运行只读案例', en: 'Step 3: Run a Read-Only Demo' },
  'qs.step4': { zh: 'Step 4: 生成你的命令', en: 'Step 4: Generate Your Own' },
  'qs.evidence.title': { zh: '维护者证据快照', en: 'Maintainer Evidence Snapshot' },
  'qs.evidence.desc': { zh: '2026-08-17，已安装的 issues.apache.org active demo 通过严格静态校验，并返回 5 条 Apache Spark 只读 issue 结果。这是可复核的日期化证据，不是 candidate package、live LLM 或第三方服务持续可用性的声明。', en: 'On 2026-08-17, the installed issues.apache.org active demo passed strict static verification and returned five read-only Apache Spark issue results. This is reproducible dated evidence, not a candidate package, live LLM, or ongoing third-party availability claim.' },
  'qs.evidence.link': { zh: '查看完整命令与边界 →', en: 'Read the captured commands and boundary →' },
  ...
  'qs.contribute.runbook': {
    zh: '推进 PyPI candidate 前，先按 <code>docs/candidate-promotion-runbook.md</code> 的 <code>Candidate Promotion Runbook</code> 跑 <code>cliany-site doctor --llm-live --require-capability generate_adapters --json</code>、adapter package、metadata validation 和 online smoke；目标包名保持 <code>pypi.org-&lt;version&gt;.cliany-adapter.tar.gz</code>。',
    en: 'Before advancing the PyPI candidate, use the <code>Candidate Promotion Runbook</code> in <code>docs/candidate-promotion-runbook.md</code> for <code>cliany-site doctor --llm-live --require-capability generate_adapters --json</code>, adapter package, metadata validation, and online smoke; keep the target package name <code>pypi.org-&lt;version&gt;.cliany-adapter.tar.gz</code>.'
  },
  ...
  'qs.maintainer.desc': {
    zh: '当前基线：v0.16.360。官网和文档现在使用显式系统字体栈，不再在运行时请求 Google Fonts；第三方字体网络故障不会延迟渲染或制造控制台错误。 HTTP <code>GET /adapters?detail=</code> 现在与 <code>GET /doctor?llm_live=</code> 共用严格布尔解析；无效值返回 <code>BAD_REQUEST</code>/400，不再静默退回简略 adapter 列表。重复布尔键同样返回 <code>BAD_REQUEST</code>/400，冲突值不再依赖 query-string 顺序。重复 <code>require_capability</code> 与 <code>domain</code> 键也返回 <code>BAD_REQUEST</code>/400，API 不会按 query-string 顺序选择能力或 adapter domain。POST /explore、POST /execute 与 POST /login 现在要求必填文本字段为非空字符串；数组、对象或空白字符串会在 SDK 前返回 <code>BAD_REQUEST</code>/400。严格 publication audit 现在有限重试短暂远端 Git 或 GitHub/PyPI JSON 传输失败；持续失败和 HTTP 响应仍然阻断，绝不把不可见产物当作已发布。新生成 adapter 现在会在 atom 执行前兑现根 --sandbox；SDK 与 POST /execute 也可显式使用同一条本地预检。Python SDK 与 GET /doctor 现在复用 CLI 的结构化 checks、summary 和稳定硬阻塞错误码；默认不会调用 provider，只有显式 llm_live=true 才请求 live preflight。Dependabot 现在将 GitHub Actions 升级合并为一个可审阅提案，减少重复的维护分支；CI 与 release workflow 现在统一使用已审阅的 actions/setup-python@v7，strict readiness 会持续检查这一 major 版本；tag Release 会直接使用已审阅的版本化 GitHub Release Notes 创建发布；严格 readiness 会在推送 tag 前阻止缺失、空白、版本不匹配或自动 compare-only 的说明。master CI 与 tag Release Preflight 现在会在发布前运行 <code>node --check site/script.js</code>。静态 adapter 检查会在读取或导入前拒绝符号链接目录、核心文件和 manifest 文件；根 CLI 直接执行 adapter 命令也会在导入 <code>commands.py</code> 前校验存在 manifest 的声明文件哈希；失配返回 <code>E_VERIFY_STATIC</code>/ <code>manifest_error</code>；这条本地边界不代表 Chrome、LLM 或第三方工作流已就绪。根 CLI 直接执行 adapter 命令现在也会在导入前运行生成模块源码安全扫描；禁用模式和非 UTF-8 模块返回本地 <code>E_VERIFY_STATIC</code>/ <code>security_issue</code>，且不会被导入。Python 与 HTTP 集成可在 browser 工作前通过 <code>ClanySite.verify(domain)</code>、同步 <code>verify(domain)</code> 或 <code>GET /verify?domain=&lt;domain&gt;</code> 运行同一静态 adapter 门禁；这不代表 Chrome、LLM 或第三方工作流已就绪。发现的 current-schema adapter 若无法注册，根 CLI 现在会返回带静态诊断详情的 <code>E_VERIFY_STATIC</code>，不再伪装为未知命令；在任何 browser 或 LLM 工作前先运行 <code>cliany-site verify &lt;domain&gt; --strict --json</code>。失败的结构化对象行现在会按字段在 <code>data.quality.field_blank_rows</code> 中列出 1 起始结果行号，成功响应与既有质量门禁不变。普通 <code>doctor</code> 现在会在展示 <code>explore</code> 前要求 live provider 预检；显式 live preflight 失败时会打印严格重试命令；缺少 key 时明确提示配置修复，E_LLM_UNAVAILABLE 时提示上游重试；默认 doctor 仍不会调用 provider。JSON 自动化中的 <code>ready_for_explore</code> 与 <code>generate_adapters.local_ready</code> 仍只表示本地配置，<code>local_blockers</code> 只解释本地前提；provider 失败时可保持 <code>local_ready=true</code>，同时整体 <code>ready=false</code>、<code>live_blockers=[&quot;llm_live&quot;]</code>。真实 <code>explore</code> 前必须要求 <code>ready_for_live_explore=true</code>，否则按严格 <code>next_step</code> 重试。candidate 晋级现在使用显式的 live-LLM 能力门禁；provider 不可用时，自动化会以非零结果停止，同时保留解释 blocker 所需的 doctor evidence。生成的 adapter 仍可重放语义化下拉选择和 Enter 提交，explore 也继续要求具名数据命令拥有通过质量检查的提取证据。显式验证未安装的 adapter 现在会返回 <code>ADAPTER_NOT_FOUND</code>，而不是空的成功结果；严格验证还会在 doctor 引导的只读命令执行前拒绝无法加载或未导出 <code>click.Group</code> <code>cli</code> 的 <code>commands.py</code> 模块。缺少可用 selector 的 extract action 现在会报告 <code>E_PARSE_FAILED</code>，不再伪装为成功。生成的 adapter 命令会保留失败 JSON envelope 并以非零状态退出；显式声明 <code>expects_nonempty=false</code> 的合法空结果仍保持成功。HTTP API 启动现在明确与 CLI 的实际传递一致：在 <code>serve</code> 前使用 <code>--headless</code> 为服务启动 Chrome，或使用 <code>--cdp-url</code> 连接已有远程浏览器；<code>GET /health</code> 用于确认可访问并返回已安装包版本。marketplace dry-run 只预检包，验证指定 domain 前必须先安装。收到 doctor evidence 的 candidate issue 模板现在会在任何可执行命令前显示当前 execution gate、adapter package 是否可运行、blocker 原因和下一步动作。candidate issue 模板会保留后续任务命令，避免人类和自动化交接在 gate 通过前把 candidate explore 表述为已就绪。公开 candidate issue body 也可审计为当前 template；出现 missing、duplicate 或 unexpected label issue 时，必须先解决再显式重写。人工审计行现在会显示 unexpected issue 的实际标题和 URL，维护者无需解析 JSON 也能定位 blocker。使用每天 1~3 个版本的发布循环、每周维护者循环、release readiness 的 next_actions、官网 alias inspect 和 PyPI 版本专属发布审计，把路线图拆成小而可验证的发布切片。doctor 会从打包的 active 案例目录选择无需登录的已发布 demo，并提供 <code>recommended_commands</code>：它使用 <code>verify --strict</code>，静态失败会以非零结果阻断顺序脚本的只读命令；安装目标已占用时从这个门禁开始，不会自动覆盖，也不把目标占用伪装为健康 adapter；它不把 candidate 或第三方在线成功伪装成已验证事实。完成归档的 <code>data.package_sha256</code> 仍是 64 个字符小写十六进制 SHA-256 摘要；可用 <code>--sha256 &lt;64-hex-sha256&gt;</code> 校验，或先运行 <code>cliany-site market install &lt;package&gt; --dry-run --json</code>。',
    en: "Current baseline: v0.16.360. The public site and docs now use explicit local system font stacks instead of requesting Google Fonts at runtime. HTTP <code>GET /adapters?detail=</code> now shares strict boolean parsing with <code>GET /doctor?llm_live=</code>; invalid values return <code>BAD_REQUEST</code>/400 instead of silently falling back to the compact adapter list. Repeated boolean query keys also return <code>BAD_REQUEST</code>/400, so conflicting values cannot depend on query-string ordering. Repeated <code>require_capability</code> and <code>domain</code> keys now also return <code>BAD_REQUEST</code>/400, so the API never chooses a capability or adapter domain by query-string order. Newly generated adapters now enforce root --sandbox before atom execution; SDK and POST /execute can opt into the same local preflight. Python SDK and GET /doctor now reuse the CLI structured checks, summary, and stable hard-blocker errors; by default they do not call a provider, and only explicit llm_live=true requests a live preflight. Dependabot now groups GitHub Actions upgrades into one reviewable proposal, reducing duplicate maintenance branches; all CI and release workflows now use reviewed actions/setup-python@v7, and strict readiness keeps that major upgrade enforced; Tag Release creates GitHub Releases from reviewed versioned notes, and strict readiness blocks missing, empty, mismatched, or generated compare-only notes before a tag is pushed. Master CI and tag Release Preflight now run <code>node --check site/script.js</code> before publication. Static adapter checks reject symbolic-link directories, core files, and manifest files before reading or importing them. Direct root CLI adapter commands also validate a present manifest's declared file hashes before importing <code>commands.py</code>; a mismatch returns <code>E_VERIFY_STATIC</code>/ <code>manifest_error</code>; this local boundary does not establish browser, LLM, or third-party workflow readiness. Root CLI dispatch now also runs the generated-module source-safety scan before import; banned patterns and non-UTF-8 modules return local <code>E_VERIFY_STATIC</code>/ <code>security_issue</code> and are not imported. Python and HTTP integrations can run the same static adapter gate with <code>ClanySite.verify(domain)</code>, synchronous <code>verify(domain)</code>, or <code>GET /verify?domain=&lt;domain&gt;</code> before browser work; this does not establish Chrome, LLM, or third-party workflow readiness. A discovered current-schema adapter that cannot register now returns structured <code>E_VERIFY_STATIC</code> details instead of an ambiguous unknown-command error; run <code>cliany-site verify &lt;domain&gt; --strict --json</code> before any browser or LLM work. Failed structured object rows now list 1-based result numbers by field in <code>data.quality.field_blank_rows</code>, while successful responses and existing quality gates stay unchanged. Ordinary human <code>doctor</code> requires a live provider preflight before it presents <code>explore</code>; if that explicit preflight fails, it prints the strict retry command; missing keys point to configuration repair and E_LLM_UNAVAILABLE points to an upstream retry, while default doctor still does not call a provider. For JSON automation, <code>ready_for_explore</code> and <code>generate_adapters.local_ready</code> remain local configuration only; <code>local_blockers</code> explains only local prerequisites. A provider failure can keep <code>local_ready=true</code> while overall <code>ready=false</code> and <code>live_blockers=[&quot;llm_live&quot;]</code>; require <code>ready_for_live_explore=true</code> before a real <code>explore</code> and follow the strict <code>next_step</code> otherwise. Candidate promotion now uses an explicit live-LLM capability gate: an unavailable provider stops automation with a nonzero result while retaining the doctor evidence needed to explain the blocker. Generated adapters still replay semantic dropdown selections and Enter-based submissions, and explore continues to require quality-checked extraction evidence for every named data command. Explicit verification of an absent adapter now returns <code>ADAPTER_NOT_FOUND</code> instead of a successful empty result; strict verification also rejects a <code>commands.py</code> module that cannot load or does not export a <code>click.Group</code> named <code>cli</code> before a doctor-guided read-only command can run. Extract actions without a usable selector now report <code>E_PARSE_FAILED</code> instead of pretending success. Generated adapter commands now preserve failed JSON envelopes and exit nonzero, while an explicitly declared <code>expects_nonempty=false</code> empty result remains successful. HTTP API startup now documents the same root browser options that the CLI passes to its server: use <code>--headless</code> before <code>serve</code> to launch Chrome for the service, or <code>--cdp-url</code> before <code>serve</code> to connect an existing remote browser; <code>GET /health</code> confirms reachability and reports the installed package version. A marketplace dry run only preflights the package, so install before verifying a named domain. Candidate issue templates retain later task commands, so human and automated handoffs do not present a candidate explore as ready before its gate passes. Public candidate issue bodies can also be audited against the current template; missing, duplicate, and unexpected labeled issues must be resolved before an explicit rewrite can run. The human-readable audit line now includes an unexpected issue\'s actual title and URL, so maintainers can locate that blocker without parsing JSON. Use the 1-3 releases/day loop, weekly maintainer loop, release readiness next_actions, website alias inspect, and PyPI version-specific publication audit to turn the roadmap into small verified releases. Doctor selects a published no-login demo from the packaged active case catalog and exposes <code>recommended_commands</code>: it uses <code>verify --strict</code>, so a static failure returns nonzero and blocks a sequential read-only command; an occupied install target begins at that gate, never auto-overwrites, and is not presented as a healthy adapter. It does not present a candidate or third-party online success as verified fact. The completed archive <code>data.package_sha256</code> remains the lowercase 64-character hexadecimal SHA-256 of the completed archive; use <code>--sha256 &lt;64-hex-sha256&gt;</code> or run <code>cliany-site market install &lt;package&gt; --dry-run --json</code> first."
  },
  'qs.maintainer.writeSchema': {
  ...
  'qs.maintainer.jsonPreflight': {
    zh: '实时 LLM 预检现在请求 JSON，与 OpenAI JSON 模式一致，避免严格服务端因纯文本提示拒绝原本正常的请求。',
    en: 'Live LLM preflight now requests JSON to match OpenAI JSON mode, preventing strict providers from rejecting a healthy connection because of a plain-text prompt.'
  },
  'qs.maintainer.packageMetadata': {
    zh: '安装和 dry-run 会在替换现有 adapter 前校验 v3 元数据。历史 Jenkins demo 发布包缺少必填字段，已标记为 degraded。',
    en: 'Package installation and dry-run validate v3 metadata before replacing existing adapters. The historical Jenkins demo is degraded because its published package lacks required fields.'
  },
  'qs.maintainer.actionDispatch': {
    zh: '共享动作执行器现在会在路由后真正执行浏览器动作，dry-run 不再改变页面，继续执行模式也会记录浏览器错误；已通过本地 Chromium 表单验证。',
    en: 'Shared action replay now executes browser actions after routing, keeps dry-run free of page changes, and records browser errors even when continuing after failures. Verified against a local Chromium form.'
  },
  ...
  'qs.maintainer.sdkSession': {
    zh: '同步 <code>save_session(domain, cdp_url=..., headless=...)</code> 复用同一条已校验的浏览器 session 路径，调用方无需自行管理事件循环。',
    en: 'Synchronous <code>save_session(domain, cdp_url=..., headless=...)</code> uses the same validated browser session path without requiring callers to manage an event loop.'
  },
  'qs.maintainer.loop': {
  ...
  'qs.maintainer.actions': {
    zh: '读取 <code>python scripts/release_readiness.py --json</code> 或 <code>python scripts/check_release_cadence.py --json</code> 输出的 <code>next_actions</code>、<code>weekly_commit_cadence_ok</code>、<code>release_count_today</code>、<code>max_daily_releases</code>、<code>daily_release_limit_ok</code>、<code>daily_release_cap_blocked</code> 和 <code>daily_release_resume_date</code>；其中 <code>daily_release_capacity_remaining</code> 仅由 cadence report 提供；daily cap 暂停时，<code>release_readiness.py --json</code> 还会输出 <code>daily_release_resume_command</code> 和 <code>daily_release_resume_command_sha256</code>。发布后用 <code>python scripts/check_release_publication.py --remote --distribution --json</code> 核对 GitHub Release、<code>pypi_version</code>、<code>pypi_latest_version</code> 与 <code>pypi_release_version</code>。',
    en: 'Read <code>next_actions</code>, <code>weekly_commit_cadence_ok</code>, <code>release_count_today</code>, <code>max_daily_releases</code>, <code>daily_release_limit_ok</code>, <code>daily_release_capacity_remaining</code>, <code>daily_release_cap_blocked</code>, and <code>daily_release_resume_date</code> from <code>python scripts/release_readiness.py --json</code> or <code>python scripts/check_release_cadence.py --json</code>; only the cadence report emits <code>daily_release_capacity_remaining</code>; when the daily cap pauses a tag, <code>release_readiness.py --json</code> also exposes <code>daily_release_resume_command</code> and <code>daily_release_resume_command_sha256</code>. After release, confirm GitHub Release plus <code>pypi_version</code>, <code>pypi_latest_version</code>, and <code>pypi_release_version</code> with <code>python scripts/check_release_publication.py --remote --distribution --json</code>.'
  },
  'qs.maintainer.issueAudit': {
    zh: '如果公开 candidate issue 已经包含 Doctor Preflight Evidence 快照，<code>audit_candidate_issues.py --apply</code> 没有当前 <code>--doctor-json</code> 时会拒绝覆盖，避免真实 blocker 证据被空白占位模板抹掉。',
    en: 'When an open candidate issue already contains a Doctor Preflight Evidence snapshot, <code>audit_candidate_issues.py --apply</code> refuses to replace it without the current <code>--doctor-json</code>, protecting real blocker evidence from an empty placeholder template.'
  },
  'qs.maintainer.issueAuditNetwork': {
    zh: '如果 GitHub API 或网络不可用，审计会返回可重试的 <code>E_GITHUB_UNAVAILABLE</code>；这不是 <code>missing</code>，不要在远端状态未读取时运行 <code>--apply</code>。',
    en: 'If the GitHub API or network is unavailable, the audit returns retryable <code>E_GITHUB_UNAVAILABLE</code>; this is not <code>missing</code>, so do not run <code>--apply</code> until remote state is readable.'
  },
  'qs.maintainer.issueAuditMissing': {
    zh: 'candidate issue 为 <code>missing</code> 时，审计现在会提供精确的 <code>issue_template_command</code> 以生成可审阅正文；审计保持只读，绝不会创建公网 issue。',
    en: 'A <code>missing</code> candidate issue now includes an exact <code>issue_template_command</code> for a reviewable body; the audit remains read-only and never creates a public issue.'
  },
  'qs.maintainer.issueAuditGraphqlEof': {
  ...
  'qs.maintainer.issueAuditRateLimit': {
    zh: 'GitHub 限流（包括 HTTP <code>429</code>、限流型 <code>403</code> 和 secondary rate limit）会报告可重试的 <code>E_GITHUB_UNAVAILABLE</code>；普通权限失败仍保持独立。',
    en: 'GitHub rate limits, including HTTP <code>429</code>, rate-limit <code>403</code>, and secondary rate limits, are retryable <code>E_GITHUB_UNAVAILABLE</code>; ordinary permission failures remain distinct.'
  },
  'qs.maintainer.issueAuditRetryHint': {
    zh: '如果 GitHub 返回 <code>Retry-After</code> 或限流 reset 时间戳，结构化重试错误会保留该提示；它不会变成 <code>missing</code>，也不会放行 <code>--apply</code>。',
    en: 'When GitHub provides <code>Retry-After</code> or a rate-limit reset timestamp, the structured retry error preserves that hint; it never becomes <code>missing</code> or permits <code>--apply</code>.'
  },
  ...
  'usecases.case3.desc': {
    zh: '2026-09-22 实测：Jira 返回 5 条 issue；Wiki 搜索返回空结果待核查；Jenkins 历史包元数据不合格，已暂停推荐。下方为历史命令示例。',
    en: 'September 22 audit: Jira returned five issues; Wiki search returned no results and needs investigation. The historical Jenkins package is degraded due to invalid metadata. Commands below are historical examples.'
  },
