const I18N = {
  ...
  'qs.contribute.case': {
    zh: '使用 <code>Real Demo Case Proposal</code> issue 模板提交公开、只读工作流；candidate 晋级 issue 可由 <code>cliany-site cases --case-id pypi-project-search --issue-template</code> 生成，并包含 <code>Acceptance Criteria</code>、<code>Primary Runbook</code> 和 <code>Doctor Preflight Evidence Fields</code>。',
    en: 'Use the <code>Real Demo Case Proposal</code> issue template for public, read-only workflows; generate candidate promotion issues with <code>cliany-site cases --case-id pypi-project-search --issue-template</code>, including <code>Acceptance Criteria</code>, <code>Primary Runbook</code>, and <code>Doctor Preflight Evidence Fields</code>.'
  },
  'qs.contribute.validate': {
    zh: '候选案例应关联 <code>cases/manifest.json</code>、<code>cases/examples/</code>、<code>python scripts/validate_cases.py --strict</code>、带 <code>llm_live_preflight</code> 的 <code>cliany-site cases --status candidate --promotion-plan</code>、带 <code>preflight_required</code> / <code>preflight_blocker</code> / <code>runbook_first</code> 的 human <code>cliany-site cases --status candidate</code> 输出、带 <code>summary.llm_live_preflight</code> 与 CDP 阻塞证据的 doctor JSON、带 <code>primary_next_task_runbook</code>、<code>primary_next_task_acceptance_criteria</code> 与 <code>doctor_preflight_evidence_fields</code> 的 <code>cliany-site cases --case-id &lt;id&gt; --evidence-bundle --json</code>，以及带 <code>Primary Acceptance Criteria</code>、<code>case_promotion_evidence_primary_llm_live_preflight_required</code>、<code>case_promotion_evidence_primary_llm_live_preflight_blocker_comment</code>、<code>case_promotion_evidence_primary_doctor_preflight_blocker_comment</code>、<code>doctor_preflight_evidence_fields</code> 与 <code>case_promotion_evidence_primary_runbook_steps</code> / hash 漂移检查的 <code>python scripts/plan_next_iteration.py --issues-dir /tmp/cliany-candidate-issues</code>。',
    en: 'Candidate cases should point to <code>cases/manifest.json</code>, <code>cases/examples/</code>, <code>python scripts/validate_cases.py --strict</code>, <code>cliany-site cases --status candidate --promotion-plan</code> with <code>llm_live_preflight</code>, human <code>cliany-site cases --status candidate</code> output with <code>preflight_required</code>, <code>preflight_blocker</code>, and <code>runbook_first</code>, doctor JSON with <code>summary.llm_live_preflight</code> and CDP blocker evidence, <code>cliany-site cases --case-id &lt;id&gt; --evidence-bundle --json</code> with <code>primary_next_task_runbook</code>, <code>primary_next_task_acceptance_criteria</code>, and <code>doctor_preflight_evidence_fields</code>, and <code>python scripts/plan_next_iteration.py --issues-dir /tmp/cliany-candidate-issues</code> with <code>Primary Acceptance Criteria</code>, <code>case_promotion_evidence_primary_llm_live_preflight_required</code>, <code>case_promotion_evidence_primary_llm_live_preflight_blocker_comment</code>, <code>case_promotion_evidence_primary_doctor_preflight_blocker_comment</code>, <code>doctor_preflight_evidence_fields</code>, and <code>case_promotion_evidence_primary_runbook_steps</code> / hash drift checks.'
  },
  ...
  'qs.maintainer.desc': {
    zh: '当前基线：v0.16.253。使用每天 1~3 个版本的发布循环、每周维护者循环、release readiness 的 next_actions、官网 alias inspect 和 PyPI 版本专属发布审计，把路线图拆成小而可验证的发布切片。',
    en: 'Current baseline: v0.16.253. Use the 1-3 releases/day loop, weekly maintainer loop, release readiness next_actions, website alias inspect, and PyPI version-specific publication audit to turn the roadmap into small verified releases.'
  },
  'qs.maintainer.loop': {
