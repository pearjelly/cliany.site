const I18N = {
  ...
  'qs.contribute.validate': {
    zh: '候选案例应关联 <code>cases/manifest.json</code>、<code>cases/examples/</code>、<code>python scripts/validate_cases.py --strict</code>、带 <code>llm_live_preflight</code> 的 <code>cliany-site cases --status candidate --promotion-plan</code>、带 <code>preflight_required</code> / <code>preflight_blocker</code> / <code>runbook_first</code> 的 human <code>cliany-site cases --status candidate</code> 输出、带 <code>summary.llm_live_preflight</code> 阻塞证据的 doctor JSON、带 <code>primary_next_task_runbook</code> 和 <code>primary_next_task_acceptance_criteria</code> 的 <code>cliany-site cases --json</code>，以及带 <code>Primary Acceptance Criteria</code>、<code>case_promotion_evidence_primary_llm_live_preflight_required</code> 与 <code>case_promotion_evidence_primary_runbook_steps</code> / hash 漂移检查的 <code>python scripts/plan_next_iteration.py --issues-dir /tmp/cliany-candidate-issues</code>。',
    en: 'Candidate cases should point to <code>cases/manifest.json</code>, <code>cases/examples/</code>, <code>python scripts/validate_cases.py --strict</code>, <code>cliany-site cases --status candidate --promotion-plan</code> with <code>llm_live_preflight</code>, human <code>cliany-site cases --status candidate</code> output with <code>preflight_required</code>, <code>preflight_blocker</code>, and <code>runbook_first</code>, doctor JSON with <code>summary.llm_live_preflight</code> blocker evidence, <code>cliany-site cases --json</code> with <code>primary_next_task_runbook</code> and <code>primary_next_task_acceptance_criteria</code>, and <code>python scripts/plan_next_iteration.py --issues-dir /tmp/cliany-candidate-issues</code> with <code>Primary Acceptance Criteria</code>, <code>case_promotion_evidence_primary_llm_live_preflight_required</code>, and <code>case_promotion_evidence_primary_runbook_steps</code> / hash drift checks.'
  },
