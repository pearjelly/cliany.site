const I18N = {
  ...
  'try.cases.body': { zh: '当 doctor 报告 ready_for_demo_adapters=true 时，按 demo_adapter_quickstart.recommended_commands 安装（仅在未安装时）、verify --strict；只有成功时才执行只读 active demo。否则浏览公开案例及其验证路径。', en: 'When doctor reports ready_for_demo_adapters=true, follow demo_adapter_quickstart.recommended_commands: install only when absent, then verify --strict; run a read-only active demo only after it succeeds. Otherwise browse public cases and their validation paths.' },
