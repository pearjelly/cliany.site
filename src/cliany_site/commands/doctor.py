...
def _print_doctor_human(result: Envelope) -> None:
    ...
    if required_capability:
        ...
        elif isinstance(live_preflight, dict) and live_preflight.get("checked") is True and (
            live_preflight.get("ready") is False
        ):
            click.echo(
                "- 修复后重新执行严格预检："
                "cliany-site doctor --llm-live --require-capability generate_adapters"
...
async def _run_llm_live_check(has_llm: bool, provider: str) -> dict[str, Any]:
    ...
    try:
        ...
        await _invoke_llm_with_retry(
            llm,
            'Reply with a JSON object only (valid json): {"status":"ok"}.',
            max_attempts=1,
            base_delay=0,
            backoff_factor=1,
        )
