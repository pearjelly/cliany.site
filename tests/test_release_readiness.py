...
def test_release_readiness_json_includes_next_actions_when_blocked(tmp_path):
    ...
    assert payload["standard_release_flow_primary_blocked_step_action_sha256"] is None
    ...
    assert payload["standard_release_flow_primary_pending_step_action"] == (
        primary_pending_step["action"]
    )
    assert payload["standard_release_flow_primary_pending_step_action_sha256"] == (
        release_readiness._stable_json_sha256(primary_pending_step["action"])
    )
...
def test_release_readiness_writes_markdown_report(tmp_path):
    ...
    assert "- standard_release_flow_primary_pending_step_action_sha256: `" in text
...
def test_release_readiness_blocks_new_target_tag_at_daily_cap(tmp_path, capsys):
    ...
    assert not any(
        "create a new release tag at HEAD"
        in action for action in payload["publication_next_actions"]
    )
    assert not any(
        "create a new release tag at HEAD"
...
def test_release_readiness_text_output_omits_next_actions_when_ready(tmp_path, capsys):
    ...
    assert "standard_release_flow_primary_pending_step_action_sha256:" in output
...
def test_release_readiness_blocks_stale_public_issue_label_safety_contract(tmp_path):
    ...
    weekly_path = repo / "docs" / "weekly-maintainer-loop.md"
    weekly_path.write_text(
        weekly_path.read_text(encoding="utf-8")
        .replace("required_labels\n", "")
        .replace("required_label_count\n", "")
        .replace("required_labels_sha256\n", ""),
        encoding="utf-8",
    )
    for readme_path in (repo / "README.md", repo / "README.zh.md"):
    ...
    _git(
        ...
        "commit",
        "-m",
        "add stale public label safety docs",
        env={
            "GIT_AUTHOR_NAME": "Test",
            "GIT_AUTHOR_EMAIL": "test@example.com",
            "GIT_COMMITTER_NAME": "Test",
    ...
    assert report.ok is False
    assert "project metadata validation failed" in report.blockers
    assert "open source metadata file missing snippet: README.md: required_labels" in (
        report.project_metadata.issues
    )
    assert "open source metadata file missing snippet: README.zh.md: required_labels" in (
        report.project_metadata.issues
    )
    assert (
    ...
    assert "open source metadata file missing snippet: site/index.html: required_labels" in (
        report.project_metadata.issues
    )
    assert "open source metadata file missing snippet: site/docs/index.html: required_labels" in (
        report.project_metadata.issues
    )
...
def test_release_readiness_blocks_stale_planner_issue_body_preflight_state_contract(tmp_path):
    repo = _init_repo(tmp_path, with_draft=True)
...
def test_release_readiness_blocks_stale_planner_handoff_preflight_state_contract(tmp_path):
    repo = _init_repo(tmp_path, with_draft=True)
    script_path = repo / "scripts" / "plan_next_iteration.py"
    script_path.parent.mkdir(parents=True, exist_ok=True)
    script_path.write_text(
    ...
    _git(
        ...
        "commit",
        "-m",
        "add stale planner handoff fixture",
        env={
            "GIT_AUTHOR_NAME": "Test",
            "GIT_AUTHOR_EMAIL": "test@example.com",
            "GIT_COMMITTER_NAME": "Test",
...
def test_release_readiness_blocks_missing_planner_daily_resume_contract(tmp_path):
    repo = _init_repo(tmp_path, with_draft=True)
    script_path = repo / "scripts" / "plan_next_iteration.py"
    script_path.parent.mkdir(parents=True, exist_ok=True)
    script_path.write_text(
    ...
    _git(
        ...
        "commit",
        "-m",
        "add planner without daily resume handoff fixture",
        env={
            "GIT_AUTHOR_NAME": "Test",
            "GIT_AUTHOR_EMAIL": "test@example.com",
            "GIT_COMMITTER_NAME": "Test",
...
def test_release_readiness_blocks_stale_weekly_loop_daily_resume_contract(tmp_path):
    repo = _init_repo(tmp_path, with_draft=True)
    (repo / "docs" / "weekly-maintainer-loop.md").write_text(
        "# 每周维护者循环\n\n"
        "python scripts/release_readiness.py --json\n"
