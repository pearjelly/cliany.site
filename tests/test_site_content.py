...
def test_site_quickstart_matches_v0150_ten_minute_success_path():
    ...
    assert "Current baseline: v0.16.350" in index
    ...
    assert "当前基线：v0.16.350" in script
    ...
    assert 'en: "Current baseline: v0.16.350.' in script
    assert "en: 'Current baseline: v0.16.350." not in script
    ...
    assert "v0.16.350 · Python" in docs
    assert "GET /adapters?detail=" in docs
    assert "BAD_REQUEST" in docs
    assert "各只能出现一次" in docs
    assert "require_capability</code> 与 <code>domain</code> 各只能出现一次" in docs
    assert "重复 domain 返回 <code>BAD_REQUEST</code> / <code>400</code>" in docs
    assert "持续失败和 HTTP 响应仍是阻断证据" in docs
    assert "fonts.googleapis.com" not in index
    assert "fonts.gstatic.com" not in index
    assert "fonts.googleapis.com" not in docs
    assert "fonts.gstatic.com" not in docs
    assert "system-ui" in styles
    assert "ui-monospace" in styles
    assert "E_SANDBOX_VIOLATION" in docs
    assert "root --sandbox" in index
    ...
    assert "rate-limit <code>403</code>" in index
    assert "限流型" in script
    assert "Retry-After" in index
    assert "reset 时间戳" in script
    ...
    assert "local_ready=true" in docs
