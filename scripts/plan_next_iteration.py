...
def _doctor_preflight_state_from_values(
    ...
    failures = [
        ...
        for check_ok, reason_code, reason in checks
        if not check_ok
    ]
    if failures:
        return {
...
class CandidatePromotion:
    ...
    doctor_preflight_evidence: dict[str, Any]
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            **self.doctor_preflight_evidence,
...
class IterationPlan:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        primary_promotion = _primary_candidate_promotion(
            self.candidate_promotions, primary_next_task
        )
        primary_task_name = (
        ...
        standard_release_flow_primary_blocked_step_handoff = (
            _standard_release_flow_primary_step_handoff(
        ...
        return {
            ...
            "case_promotion_evidence_primary_issue_template_command": (
                primary_promotion.issue_template_command if primary_promotion else None
            ),
            "case_promotion_evidence_primary_issue_template_json_command": (
                primary_promotion.issue_template_json_command if primary_promotion else None
            ),
            "case_promotion_evidence_primary_evidence_bundle_command": (
                primary_promotion.evidence_bundle_command if primary_promotion else None
            ),
            "case_promotion_evidence_primary_evidence_bundle_json_command": (
                primary_promotion.evidence_bundle_json_command if primary_promotion else None
            ),
            ...
            **_primary_doctor_preflight_aliases(primary_next_task),
            "case_promotion_llm_live_preflight_evidence_fields": (
                llm_live_preflight_evidence_fields
            ),
...
def _primary_candidate_promotion(
    ...
    if not primary_next_task:
        return None
    case_id = primary_next_task.get("case_id")
    if not case_id:
        return None
    return next(
        (promotion for promotion in promotions if promotion.case_id == str(case_id)),
        None,
    )
...
def _standard_release_flow_distribution_audit_command(flow: dict[str, Any]) -> str | None:
    ...
    for command in commands:
        ...
        if (
            "scripts/check_release_publication.py" in command_parts
            and "--remote" in command_parts
            and "--distribution" in command_parts
...
def _standard_release_flow_primary_step_handoff_aliases(
    flow: dict[str, Any],
) -> dict[str, str | None]:
    aliases: dict[str, str | None] = {}
    for prefix in ("blocked", "pending"):
        handoff = _standard_release_flow_primary_step_handoff(flow, prefix)
        aliases.update(
            {
                ...
                f"standard_release_flow_primary_{prefix}_step_command_sha256": handoff[
                    "command_sha256"
                ],
                f"standard_release_flow_primary_{prefix}_step_action": handoff[
                    "action"
                ],
                f"standard_release_flow_primary_{prefix}_step_action_sha256": handoff[
                    "action_sha256"
                ],
            }
        )
...
def _task_with_doctor_preflight_evidence_fields(task: Any) -> Any:
    ...
    if required is True:
        command_text = str(
            normalized.get("llm_live_preflight_command")
    ...
    *,
    ...
    for priority_rank, case in enumerate(ranked_candidate_cases, start=1):
        ...
        if evidence_bundle_primary_next_task:
            ...
            if doctor_preflight_evidence and evidence_bundle_primary_next_task.get(
                "llm_live_preflight_required"
            ):
                evidence_bundle_primary_next_task.update(doctor_preflight_evidence)
        evidence_bundle_primary_next_task_runbook = _candidate_primary_task_runbook(
            evidence_bundle_primary_next_task,
            promotion_command_plan,
        )
        issue_template_command = _candidate_issue_template_command(
            str(case.id),
            doctor_preflight_evidence=doctor_preflight_evidence,
        )
        issue_template_json_command = _candidate_issue_template_json_command(
            str(case.id),
            doctor_preflight_evidence=doctor_preflight_evidence,
        )
        ...
        promotions.append(
            CandidatePromotion(
                ...
                doctor_preflight_evidence=dict(doctor_preflight_evidence or {}),
...
def _candidate_evidence_bundle_command(
    case_id: str,
    *,
    doctor_preflight_evidence: dict[str, Any] | None = None,
) -> str:
...
def _primary_llm_live_preflight_aliases(primary_task: dict[str, Any] | None) -> dict[str, Any]:
    ...
    command_text = str(command) if command else None
...
def _primary_doctor_preflight_aliases(primary_task: dict[str, Any] | None) -> dict[str, Any]:
    task = primary_task if isinstance(primary_task, dict) else {}
    required = task.get("llm_live_preflight_required")
    ...
    *,
    ...
    if doctor_preflight_evidence:
        ...
        current_doctor_preflight_section = [
            ...
            "",
            "## Current Doctor Preflight Evidence Values",
    ...
    return "\n".join(
        [
            ...
            *task_lines,
            "",
            "## Evidence Bundle",
            f"- Human: `{evidence_bundle_command}`",
            f"- JSON: `{evidence_bundle_json_command}`",
...
def build_plan(
    ...
    *,
    ...
    candidate_promotions = _candidate_promotions(
        readiness,
        doctor_preflight_evidence=doctor_preflight_evidence,
    )
...
def _print_text(plan: IterationPlan) -> None:
    ...
    standard_release_flow_primary_step_handoff_aliases = (
        _standard_release_flow_primary_step_handoff_aliases(plan.standard_release_flow)
    )
    website_deploy_command = _standard_release_flow_website_deploy_command(
        plan.standard_release_flow
    )
    ...
    print(
        "standard_release_flow_has_distribution_audit: "
        f"{distribution_audit_command is not None}"
    )
    print(
        "standard_release_flow_distribution_audit_command: "
    ...
    print(
        "standard_release_flow_distribution_audit_command_sha256: "
        f"{_stable_json_sha256(distribution_audit_command) if distribution_audit_command else None}"
    )
    ...
    if isinstance(primary_next_task, dict) and primary_next_task:
        ...
        for key, value in primary_next_task.items():
            print(f"  {key}: {value}")
        primary_promotion = _primary_candidate_promotion(
            plan.candidate_promotions, primary_next_task
        )
        ...
        if primary_promotion:
            print(
                "case_promotion_evidence_primary_issue_template_command: "
                f"{primary_promotion.issue_template_command}"
            )
            print(
                "case_promotion_evidence_primary_issue_template_json_command: "
                f"{primary_promotion.issue_template_json_command}"
            )
            print(
                "case_promotion_evidence_primary_evidence_bundle_command: "
                f"{primary_promotion.evidence_bundle_command}"
            )
            print(
                "case_promotion_evidence_primary_evidence_bundle_json_command: "
                f"{primary_promotion.evidence_bundle_json_command}"
            )
...
def _render_markdown(plan: IterationPlan) -> str:
    ...
    primary_candidate_promotion = _primary_candidate_promotion(
        plan.candidate_promotions,
    ...
    primary_issue_template_command = _format_context_value(
        primary_candidate_promotion.issue_template_command
        if primary_candidate_promotion
        else None
    )
    primary_issue_template_json_command = _format_context_value(
        primary_candidate_promotion.issue_template_json_command
        if primary_candidate_promotion
        else None
    )
    primary_evidence_bundle_command = _format_context_value(
        primary_candidate_promotion.evidence_bundle_command
        if primary_candidate_promotion
        else None
    )
    primary_evidence_bundle_json_command = _format_context_value(
        primary_candidate_promotion.evidence_bundle_json_command
        if primary_candidate_promotion
        else None
    )
    ...
    primary_preflight_command_sha256_text = _format_context_value(
        primary_preflight_aliases[
            "case_promotion_evidence_primary_llm_live_preflight_command_sha256"
        ]
    )
    ...
    primary_doctor_preflight_aliases = _primary_doctor_preflight_aliases(
        primary_candidate_task if isinstance(primary_candidate_task, dict) else None
    )
    primary_doctor_preflight_blocker_comment_text = _summary_inline_code(
    ...
    standard_release_flow_primary_step_handoff_text = {
        key: _format_context_value(value)
        for key, value in standard_release_flow_primary_step_handoff_aliases.items()
    }
    ...
    blocked_step_command_sha256 = standard_release_flow_primary_step_handoff_text[
        "standard_release_flow_primary_blocked_step_command_sha256"
    ]
    blocked_step_action = standard_release_flow_primary_step_handoff_text[
        "standard_release_flow_primary_blocked_step_action"
    ]
    blocked_step_action_sha256 = standard_release_flow_primary_step_handoff_text[
        "standard_release_flow_primary_blocked_step_action_sha256"
    ]
    ...
    pending_step_command_sha256 = standard_release_flow_primary_step_handoff_text[
        "standard_release_flow_primary_pending_step_command_sha256"
    ]
    pending_step_action = standard_release_flow_primary_step_handoff_text[
        "standard_release_flow_primary_pending_step_action"
    ]
    pending_step_action_sha256 = standard_release_flow_primary_step_handoff_text[
        "standard_release_flow_primary_pending_step_action_sha256"
    ]
    standard_release_flow_website_deploy_command = (
    ...
    standard_release_flow_distribution_audit_command_text = _format_context_value(
        standard_release_flow_distribution_audit_command
    )
    standard_release_flow_distribution_audit_command_sha256_text = _format_context_value(
        standard_release_flow_distribution_audit_command_sha256
    )
    distribution_audit_present_text = str(
        standard_release_flow_distribution_audit_command is not None
    ).lower()
    distribution_audit_command_text = standard_release_flow_distribution_audit_command_text
    distribution_audit_sha256_text = (
        standard_release_flow_distribution_audit_command_sha256_text
    )
...
| standard_release_flow_primary_pending_step_action_sha256 | `{pending_step_action_sha256}` |
...
| case_promotion_evidence_primary_issue_template_command | `{primary_issue_template_command}` |
| case_promotion_evidence_primary_issue_template_json_command | `{primary_issue_template_json_command}` |
| case_promotion_evidence_primary_evidence_bundle_command | `{primary_evidence_bundle_command}` |
| case_promotion_evidence_primary_evidence_bundle_json_command | `{primary_evidence_bundle_json_command}` |
| case_promotion_evidence_primary_runbook_step_count | `{len(primary_runbook_steps)}` |
...
| case_promotion_evidence_primary_llm_live_preflight_command_sha256 | `{primary_preflight_command_sha256_text}` |
...
def _standard_release_flow_markdown(flow: dict[str, Any]) -> str:
    ...
    primary_step_handoff_text = "\n".join(primary_step_handoff_lines)
        ...
        metadata.append(
            {
                ...
                **promotion.doctor_preflight_evidence,
                **_doctor_preflight_evidence_template_aliases(
                    promotion.doctor_preflight_evidence_template
                ),
...
def _render_issue_artifacts_readme(
    ...
    *,
    ...
    standard_flow_distribution_audit_command_sha256 = (
        _stable_json_sha256(standard_flow_distribution_audit_command)
        if standard_flow_distribution_audit_command
        else None
    )
    standard_flow_distribution_audit_command_text = _format_context_value(
        standard_flow_distribution_audit_command
    )
    standard_flow_distribution_audit_command_sha256_text = _format_context_value(
        standard_flow_distribution_audit_command_sha256
    )
...
def _issue_artifact_bundle_summary(
    *,
    ...
    case_promotion_evidence_primary_promotion = _primary_candidate_promotion(
        plan.candidate_promotions,
        case_promotion_evidence_primary_next_task,
    )
    ...
    return {
        ...
        **_daily_release_handoff_aliases(plan),
        ...
        "case_promotion_evidence_primary_issue_template_command": (
            case_promotion_evidence_primary_promotion.issue_template_command
            if case_promotion_evidence_primary_promotion
            else None
        ),
        "case_promotion_evidence_primary_issue_template_json_command": (
            case_promotion_evidence_primary_promotion.issue_template_json_command
            if case_promotion_evidence_primary_promotion
            else None
        ),
        "case_promotion_evidence_primary_evidence_bundle_command": (
            case_promotion_evidence_primary_promotion.evidence_bundle_command
            if case_promotion_evidence_primary_promotion
            else None
        ),
        "case_promotion_evidence_primary_evidence_bundle_json_command": (
            case_promotion_evidence_primary_promotion.evidence_bundle_json_command
            if case_promotion_evidence_primary_promotion
            else None
        ),
        ...
        **_primary_doctor_preflight_aliases(case_promotion_evidence_primary_next_task),
        "case_promotion_llm_live_preflight_evidence_field_count": len(
            llm_live_preflight_evidence_fields
        ),
    ...
    *,
    ...
    return "\n".join(
        [
            ...
            "- case_promotion_evidence_primary_issue_template_command: "
            f"{_summary_inline_code(summary['case_promotion_evidence_primary_issue_template_command'])}",
            "- case_promotion_evidence_primary_issue_template_json_command: "
            f"{_summary_inline_code(summary['case_promotion_evidence_primary_issue_template_json_command'])}",
            "- case_promotion_evidence_primary_evidence_bundle_command: "
            f"{_summary_inline_code(summary['case_promotion_evidence_primary_evidence_bundle_command'])}",
            "- case_promotion_evidence_primary_evidence_bundle_json_command: "
            f"{_summary_inline_code(summary['case_promotion_evidence_primary_evidence_bundle_json_command'])}",
            "- case_promotion_evidence_primary_evidence_sha256: "
            ...
            "- case_promotion_evidence_primary_llm_live_preflight_command_sha256: "
            f"`{summary['case_promotion_evidence_primary_llm_live_preflight_command_sha256']}`",
            "- case_promotion_evidence_primary_llm_live_preflight_blocker_note: "
            ...
            "- standard_release_flow_primary_blocked_step_command_sha256: "
            f"{_summary_inline_code(summary['standard_release_flow_primary_blocked_step_command_sha256'])}",
            "- standard_release_flow_primary_blocked_step_action: "
            f"{_summary_inline_code(summary['standard_release_flow_primary_blocked_step_action'])}",
            "- standard_release_flow_primary_blocked_step_action_sha256: "
            ...
            "- standard_release_flow_primary_pending_step_command_sha256: "
            f"{_summary_inline_code(summary['standard_release_flow_primary_pending_step_command_sha256'])}",
            "- standard_release_flow_primary_pending_step_action: "
            f"{_summary_inline_code(summary['standard_release_flow_primary_pending_step_action'])}",
            "- standard_release_flow_primary_pending_step_action_sha256: "
            f"{_summary_inline_code(summary['standard_release_flow_primary_pending_step_action_sha256'])}",
            "- standard_release_flow_has_website_deploy: "
            ...
            "- standard_release_flow_distribution_audit_command: "
            f"{_summary_inline_code(summary['standard_release_flow_distribution_audit_command'])}",
            "- standard_release_flow_distribution_audit_command_sha256: "
            f"`{summary['standard_release_flow_distribution_audit_command_sha256']}`",
            "- standard_release_flow_sha256: "
...
def main(argv: list[str] | None = None) -> int:
    ...
    doctor_preflight_evidence = (
        _load_doctor_preflight_evidence(args.doctor_json)
